...
export default function HowWork({
    ...
    () => {
      ...
      mm.add("(min-width: 1024px)", () => {
        ...
        cardsRef.current.forEach((card, idx) => {
          ...
          tl.fromTo(plusFollow, { rotation: 0 }, { rotation: 360 * 2, right: -12 }, "<10%")
            .fromTo(line, { scaleX: 0, transformOrigin: "left" }, { scaleX: 1 }, "<")
            .fromTo(numbers, { autoAlpha: 0, filter: "blur(12px)", force3D: true }, { autoAlpha: 1, filter: "blur(0px)", duration: 0.2 }, "<")
            .fromTo(title,   { autoAlpha: 0, y: 20 }, { autoAlpha: 1, y: 0 }, "<25%")
            .fromTo(content, { autoAlpha: 0, y: 20 }, { autoAlpha: 0.5, y: 0 }, "<");
        ...
        if (nextSection) {
          ...
          else { tl.to({}, { duration: 1 }); gsap.set(nextSection, { marginTop: "-100dvh" }); }
        }

        if (stripeCount > 0) {
          tl.addLabel("stripes_start");
          const staggerAmount = 0.6, perStripe = 1 - staggerAmount, totalStripeDur = 1.2;
          for (let i = 0; i < stripeCount; i++) {
            const staggerIdx = stripeCount - 1 - i;
            const offset = (staggerAmount * staggerIdx) / (stripeCount - 1 || 1);
            const s = offset * totalStripeDur, e = s + perStripe * totalStripeDur;
            tl.fromTo(stripes[i]!, { scaleY: 0 }, { scaleY: 1, duration: e - s, ease: "none" }, `stripes_start+=${s}`);
          }
        }
      ...
      mm.add("(max-width: 1023px)", () => {
        ...
        cardsRef.current.forEach((card, idx) => {
          ...
          gsap.set([nums, ttl, cnt], idx === 0 ? { autoAlpha: 1, y: 0 } : { autoAlpha: 0, y: 20 });
        });
        cardLinesRef.current.forEach(el  => { if (el) gsap.set(el, { scaleX: 0, transformOrigin: "left", autoAlpha: 0 }); });
        gsap.set(stripes, { scaleY: 0, transformOrigin: "bottom" });
        ...
        cardsRef.current.forEach((card, idx) => {
          ...
          if (idx > 0) {
            ...
            mobileTl
              .fromTo(nums!, { autoAlpha: 0, y: 20 }, { autoAlpha: 1, y: 0, duration: fadeDur * 0.55 }, slotStart)
              .fromTo(ttl!,  { autoAlpha: 0, y: 20 }, { autoAlpha: 1, y: 0, duration: fadeDur * 0.55 }, slotStart + fadeDur * 0.15)
              .fromTo(cnt!,  { autoAlpha: 0, y: 20 }, { autoAlpha: 1, y: 0, duration: fadeDur * 0.55 }, slotStart + fadeDur * 0.30);
          }

          // Line draws left→right across the full slot
          if (fill) {
            mobileTl
              .set(fill,  { autoAlpha: 1, scaleX: 0, transformOrigin: "left" }, slotStart)
              .fromTo(fill, { scaleX: 0 }, { scaleX: 1, duration: slotDur }, slotStart);
          }
        });
        ...
        const tick = () => {
          ...
          const holdT   = Math.max(0, Math.min(1, (rawProg - animationEnd) / (1 - animationEnd)));
  ...
  return (
    ...
    >
      ...
      <div ref={sectionRef} className="lg:py-37.5 pt-6 pb-4 relative h-dvh lg:h-auto lg:min-h-dvh overflow-hidden lg:overflow-visible">
        <div className="tr__container relative w-full grid grid-cols-12 gap-x-6">

          {/* ── Header ── */}
          <div className="grid grid-cols-12 gap-x-6 lg:mb-20 mb-4 col-span-12">

            {/* Mobile/tablet header */}
            <div className="col-span-12 mt-16 lg:hidden">
              <BlurTextReveal
                as="span"
                text="Our process"
                animationType="chars"
              ...
              <BlurTextReveal
                ...
                className="mb-3"
              />
              <p className="small opacity-50 max-w-80">
            ...
            <BlurTextReveal
              ...
              animationType="chars"
              stagger={0.05}
              className="title hidden lg:block col-span-2 pt-4"
            />
          ...
          {/* ── Cards ── */}
          <div className="col-span-12 lg:col-span-9 lg:col-start-3 mt-6 gap-x-6 relative">
            <div className="grid grid-cols-1 lg:grid-cols-12">
              {cards.map((card, idx) => (
                <div
                  ref={(self) => { if (self) cardsRef.current[idx] = self; }}
                  key={idx}
                  className="col-span-1 lg:col-span-4 relative lg:pb-20 pb-7 lg:pr-6 lg:last:pr-0 [&:not(:first-child)]:mt-10 lg:[&:not(:first-child)]:mt-0"
                >
                  {/* <div className="flex lg:block items-baseline gap-x-4 lg:gap-x-0 lg:mb-0 mb-2">
                    <span className="title step-no lg:mb-9 lg:block shrink-0 opacity-50">
                      Step - {card.id}
                    </span>
                    <h3 className="work-title lg:mb-8 lg:mt-9">{card.title}</h3>
                  </div>
                  ...
                  <div className="flex lg:block items-baseline gap-x-4 lg:gap-x-0 lg:mb-0 mb-2">
                    ...
                    <div className="flex-1 lg:block">
                      <h3 className="work-title lg:mb-8 lg:mt-9">
                        {card.title}
                      </h3>

                      <p className="content small max-w-87.5 mt-2 lg:mt-0">
                        {card.content}
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="fill-line absolute left-0 bottom-0 w-full hidden lg:block">
                    <div
                    ...
                    <svg width="13" height="13" viewBox="0 0 13 13" fill="none"
                      ...
                      <line x1="6.5" y1="0" x2="6.5" y2="13" stroke={strokeColor} />
                      <line x1="13" y1="6.5" x2="0" y2="6.5" stroke={strokeColor} />
                    </svg>
                    <svg width="13" height="13" viewBox="0 0 13 13" fill="none"
                  ...
                  {idx !== cards.length - 1 && (
                  <div className="absolute left-0 bottom-0 w-full h-px lg:hidden">
                    {/* bright fill — scaleX 0→1 scrubbed */}
                    <div
                      ref={(el) => { if (el) cardLinesRef.current[idx] = el; }}
