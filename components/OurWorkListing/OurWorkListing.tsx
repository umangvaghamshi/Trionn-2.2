...
const PROJECTS: Project[] = [
  { pos: 'left',   size: 'large',  title: 'Velora Studio',   desc: 'Architecture design\nfor a modern brand.',   year: '2025', img: '/assets/orbit-01.jpg', link: true },
  { pos: 'right',  size: 'medium', title: 'Northform™',      desc: 'Brand identity system\nand visual language.', year: '2025', img: '/assets/orbit-02.jpg' },
  { pos: 'center', size: 'xlarge', title: 'Neue Montreal',   desc: 'Typography system\nfor digital products.',    year: '2024', img: '/assets/orbit-03.jpg', link: true },
  { pos: 'left',   size: 'small',  title: 'One.Dot',         desc: 'Tailored website design\nfor refined brands.',year: '2025', img: '/assets/orbit-04.jpg' },
  { pos: 'right',  size: 'large',  title: 'Axora Ceramics',  desc: 'Brand identity and\neditorial design.',       year: '2025', img: '/assets/orbit-05.jpg', link: true },
  { pos: 'left',   size: 'medium', title: 'Grinex Projects', desc: 'Sustainable material\nbrand system.',         year: '2025', img: '/assets/orbit-07.jpg' },
  { pos: 'center', size: 'xlarge', title: 'Royal Festival',  desc: 'Visual identity for\nan event series.',       year: '2024', img: '/assets/orbit-08.jpg', link: true },
  { pos: 'right',  size: 'small',  title: 'Formline Studio', desc: 'Interface design for\nselected work.',        year: '2025', img: '/assets/orbit-09.jpg' },
  { pos: 'left',   size: 'large',  title: 'Molson Coors',    desc: 'Office building\narchitecture design.',       year: '2024', img: '/assets/orbit-01.jpg', link: true },
  { pos: 'right',  size: 'medium', title: 'Dark Identity',   desc: 'Branding and digital\nsystems at scale.',     year: '2025', img: '/assets/orbit-02.jpg' },
  { pos: 'center', size: 'xlarge', title: 'Typeform Lab',    desc: 'Font design and\ntype specimen work.',        year: '2024', img: '/assets/orbit-03.jpg', link: true },
  { pos: 'left',   size: 'small',  title: 'Refined Co.',     desc: 'Minimal website for\na luxury consultancy.',  year: '2025', img: '/assets/orbit-04.jpg' },
  { pos: 'right',  size: 'large',  title: 'Bloom Studio',    desc: 'Ceramic brand identity\nand packaging design.', year: '2024', img: '/assets/orbit-05.jpg', link: true },
  { pos: 'left',   size: 'medium', title: 'Zero Point',      desc: 'Sustainable brand\nfor architecture firm.',   year: '2025', img: '/assets/orbit-07.jpg' },
  { pos: 'center', size: 'xlarge', title: 'Festival Noir',   desc: 'Event identity and\nmotion design work.',     year: '2024', img: '/assets/orbit-08.jpg', link: true },
  { pos: 'right',  size: 'large',  title: 'Selected Works',  desc: 'Portfolio curation and\nUI/UX direction.',    year: '2025', img: '/assets/orbit-09.jpg', link: true },
  { pos: 'left',   size: 'medium', title: 'Archi Collective',desc: 'Architecture studio\nbranding system.',       year: '2024', img: '/assets/orbit-01.jpg' },
  { pos: 'right',  size: 'small',  title: 'Identity Works',  desc: 'Complete brand identity\nand digital presence.', year: '2025', img: '/assets/orbit-02.jpg' },
  { pos: 'center', size: 'xlarge', title: 'Letterform',      desc: 'Custom typeface for\na global publication.',  year: '2024', img: '/assets/orbit-03.jpg', link: true },
  { pos: 'right',  size: 'medium', title: 'Dot Collective',  desc: 'Brand system for a\ncreative collective.',    year: '2025', img: '/assets/orbit-04.jpg' },
];

const ORBIT_IMGS = [
  '/assets/orbit-01.jpg', '/assets/orbit-02.jpg', '/assets/orbit-03.jpg',
  '/assets/orbit-04.jpg', '/assets/orbit-05.jpg', '/assets/orbit-07.jpg',
  '/assets/orbit-08.jpg', '/assets/orbit-09.jpg',
];
...
const FT_THUMBS = [
  ...
  '/assets/orbit-04.jpg', '/assets/orbit-05.jpg', '/assets/orbit-07.jpg',
  '/assets/orbit-08.jpg', '/assets/orbit-09.jpg',
  '/assets/orbit-01.jpg', '/assets/orbit-03.jpg', '/assets/orbit-05.jpg', '/assets/orbit-07.jpg',
];
...
export default function OurWorkListing() {
  ...
  useEffect(() => {
    ...
    const onPageShow = () => { forceTop(); requestAnimationFrame(forceTop); };
    ...
    const THUMB_W = 170, THUMB_H = 114;
    const WHEEL_NEEDED = 1600;
    const DONE_ON = 0.9995, DONE_OFF = 0.985;
    ...
    const STRIP_STAGGER = (1 - STRIP_DRAW_WINDOW) / Math.max(1, STRIP_LINE_COUNT - 1);
    let _logoSvgReady = false;
    let _fieldLines1: any[] | null = null, _fieldLines2: any[] | null = null, _fieldLines3: any[] | null = null;
    ...
    function fieldLineH(y: number, seed: number) {
      const W = 578, steps = 60, freq = 1.2 + ((seed * 0.37) % 0.8), amp = 8 + ((seed * 0.61) % 14), phase = (seed * 1.7) % (Math.PI * 2);
    ...
    function fieldLineV(x: number, seed: number) {
      const H = 586, steps = 60, freq = 1.2 + ((seed * 0.37) % 0.8), amp = 8 + ((seed * 0.61) % 14), phase = (seed * 1.7) % (Math.PI * 2);
    ...
    function genFieldLines(direction: 'top-down' | 'bottom-up' | 'left-right') {
      return Array.from({ length: STRIP_LINE_COUNT }, (_, i) => {
        ...
        if (direction === 'top-down') {
          const spacing = 586 / STRIP_LINE_COUNT, y = (i + 0.5) * spacing;
          return { path: fieldLineH(y, seed), delay: i * STRIP_STAGGER, fillX: -10, fillY: y - spacing / 2, fillW: 600, fillH: spacing + 1, axis: 'h' };
        }
        if (direction === 'bottom-up') {
          const spacing = 586 / STRIP_LINE_COUNT, y = 586 - (i + 0.5) * spacing;
          return { path: fieldLineH(y, seed), delay: i * STRIP_STAGGER, fillX: -10, fillY: y, fillW: 600, fillH: spacing + 1, axis: 'h' };
        }
        const spacing = 578 / STRIP_LINE_COUNT, x = (i + 0.5) * spacing;
        return { path: fieldLineV(x, seed), delay: i * STRIP_STAGGER, fillX: x - spacing / 2, fillY: -10, fillW: spacing + 1, fillH: 610, axis: 'v' };
      });
    }
    function ensureLogoSvg() {
      ...
      if (!_logoSvgReady) {
        [e.lg1, e.lg2, e.lg3].forEach(g => { if (g) (g as any).style.display = 'block'; });
        [e.t1, e.t2, e.t3].forEach(t => { if (t) { t.style.opacity = '0'; (t as any).style.transformOrigin = '50% 50%'; } });
    ...
    function renderSvgStrip(drawGroup: SVGGElement | null, fillGroup: SVGGElement | null, lines: any[], progress: number) {
      ...
      lines.forEach((fl: any) => {
        ...
        if (drawP <= 0) return;
        const fillP = easeInOut(clamp01((progress - fl.delay) / (STRIP_DRAW_WINDOW * 1.4)));
        if (fillP > 0) {
          if (fl.axis === 'h') fillHTML += `<rect x="${fl.fillX}" y="${fl.fillY.toFixed(1)}" width="${fl.fillW}" height="${(fl.fillH * fillP).toFixed(1)}" fill="#D0D0D0"/>`;
          else fillHTML += `<rect x="${fl.fillX.toFixed(1)}" y="-10" width="${(fl.fillW * fillP).toFixed(1)}" height="610" fill="#D0D0D0"/>`;
        }
    ...
    function resetSvgStrips() {
      const e = logoSvgEls();
      [e.pg1, e.pg2, e.pg3, e.lg1, e.lg2, e.lg3].forEach(g => { if (g) g.innerHTML = ''; });
    }
    function drawLogoIntroParts(ts: number) {
      ...
      defs.forEach(d => {
        ...
        (d.el as any).style.transform = `translate(${(d.x * inv).toFixed(2)}px,${(d.y * inv).toFixed(2)}px) rotate${d.axis}(${(d.rot * inv).toFixed(2)}deg) rotateZ(${(d.z * inv).toFixed(2)}deg) scale(${scale.toFixed(4)})`;
      });
      return allDone;
    }
    function drawLogoFill(fillProgress: number, ts?: number) {
      ...
      [e.t1, e.t2, e.t3].forEach(t => { if (t) { t.style.opacity = '1'; (t as any).style.transform = 'translate(0,0) scale(1)'; } });
      renderSvgStrip(e.lg1, e.pg1, _fieldLines1 || genFieldLines('bottom-up'), fillProgress);
      renderSvgStrip(e.lg2, e.pg2, _fieldLines2 || genFieldLines('left-right'), fillProgress);
      renderSvgStrip(e.lg3, e.pg3, _fieldLines3 || genFieldLines('top-down'), fillProgress);
    }
    ...
    function getCenter() {
      ...
      return { cx: window.innerWidth / 2 - r.left, cy: window.innerHeight / 2 - r.top - upOffset };
    }
    ...
    function orbitPos3D(td: any) {
      ...
      const baseAlpha = delayActive ? 1 : (
        age < 0.8 ? age / 0.8 :
        age > life - 1.2 ? Math.max(0, (life - age) / 1.2) : 1
      );
      const alpha = typeof td._alphaOverride === 'number' ? clamp01(td._alphaOverride) : baseAlpha;
      const screenX = (td.floatX || 0) + td.TW / 2;
      const screenY = (td.floatY || 0) + td.TH / 2;
      return { x: td.floatX || 0, y: td.floatY || 0, screenX, screenY, Z3: 0, depthN: scale, depthScale: scale, depthAlpha: alpha };
    }
    function placeThumbs() {
      ...
      let pool = [...ORBIT_IMGS, ...ORBIT_IMGS, ...ORBIT_IMGS, ...ORBIT_IMGS].slice(0, thumbs.length);
      ...
      thumbs.forEach((el, i) => {
        ...
        if (img) {
          ...
          (img as HTMLImageElement).style.cssText = 'width:100%;height:100%;object-fit:cover;display:block;';
        }
        const td: any = {
          el, TW, TH,
          warpAngle: 0, warpPhase: 0, warpSpeed: 0,
          floatX: 0, floatY: 0, floatVX: 0, floatVY: 0,
          floatScale: 0.7, floatAlpha: 0, floatAge: 0, floatLife: 8,
          R: ORBIT_R_INNER, slotAngle: 0, direction: 1, tilt: TILT_INNER, cyOffset: 0, isOuter: false,
          flyInDone: true, flyInStartTs: null, flyInStarted: true,
          _introStart: performance.now() + i * INTRO_STAGGER, _introDone: false,
          fromX: 0, fromY: 0, curX: 0, curY: 0, flyInProgress: 1,
          curRotX: 0, curRotY: 0,
        };
    ...
    function updateDotPos() {
      ...
      if (h) { const r = h.getBoundingClientRect(); dotX -= r.left; dotY -= r.top; }
    }
    ...
    function applyThumb3D(td: any, pos: any, scaleMulti?: number, alphaMulti?: number) {
      ...
      const mt = (window as any)._mouseTilt || { mx: window.innerWidth / 2, my: window.innerHeight / 2 };
      ...
      if (td._lastA !== aClamped) { td.el.style.opacity = String(aClamped); td._lastA = aClamped; }
      const z = Math.round(pos.depthN * 15 + 1);
      if (td._lastZ !== z) { td.el.style.zIndex = String(z); td._lastZ = z; }
    }
    ...
    function loop(ts: number) {
      ...
      if (raw < FLOAT_RESUME_AT) { raw = 0; wheelAccum = 0; wheelTarget = 0; }
      rawSmooth = raw;
      if (raw > FLOAT_RESUME_AT) topRerenderPausedUntil = Math.max(topRerenderPausedUntil, ts + 1);
      const enteringConverge = raw > FLOAT_RESUME_AT && !convergeLocked;
      const leavingConverge  = raw <= FLOAT_RESUME_AT && convergeLocked;
      orbitTime += dt;
      const allLanded = thumbData.length > 0 && thumbData.every(td => td.flyInDone);
      ...
      }
      {
        ...
        if (dot) dot.style.transform = `translate3d(-50%, -50%, 0) scale(1) perspective(800px) rotateX(${logoTiltRX.toFixed(2)}deg) rotateY(${logoTiltRY.toFixed(2)}deg)`;
      }
      waveTime += dt;
      ...
      for (let i = 0; i < thumbData.length; i++) {
        ...
        td.floatAge += dt;
        const vw = window.innerWidth, vh = window.innerHeight;
        const offscreen = td.floatX < -td.TW * 2 || td.floatX > vw + td.TW * 2 ||
                          td.floatY < -td.TH * 2 || td.floatY > vh + td.TH * 2;
        const allowTopRerender = raw <= 0.001 && ts >= topRerenderPausedUntil;
        const needsRespawn = allowTopRerender && (td.floatAge >= td.floatLife || offscreen);
        if (needsRespawn || td._respawnState) {
          if (!td._respawnState) {
            const currentAlpha = (typeof td._lastA === 'number') ? td._lastA : 1;
            td._respawnState = { phase: 'out', start: ts, fromAlpha: currentAlpha };
          }
      ...
      if (leavingConverge) {
        ...
        thumbData.forEach((td, idx) => {
          ...
          delete td._moveFromX; delete td._moveFromY;
          delete td._moveFromScale; delete td._moveFromAlpha; delete td._moveFromDepth;
          delete td._lockFloatX; delete td._lockFloatY;
          delete td._respawnState; delete td._alphaOverride;
        });
        convergeLocked = false;
      }
      thumbData.forEach((td, idx) => {
        if (td.el.style.visibility === 'hidden') { td.el.style.visibility = 'visible'; td.el.style.transition = ''; }
        if (!td.flyInStarted) { td.el.style.opacity = '0'; return; }
        const livePos = livePosArr[idx];
        if (raw > 0) {
          const ep = getThumbEp(idx, rawSmooth);
          const cx2 = dotX - td.TW / 2, cy2 = dotY - td.TH / 2;
          const driftX = (typeof td._lockFloatX === 'number') ? (td.floatX - td._lockFloatX) : 0;
          const driftY = (typeof td._lockFloatY === 'number') ? (td.floatY - td._lockFloatY) : 0;
          const fromX = (typeof td._moveFromX === 'number') ? td._moveFromX + driftX : livePos.x;
          const fromY = (typeof td._moveFromY === 'number') ? td._moveFromY + driftY : livePos.y;
          const fromScale = (typeof td._moveFromScale === 'number') ? td._moveFromScale : livePos.depthScale;
          const fromAlpha = (typeof td._moveFromAlpha === 'number') ? td._moveFromAlpha : livePos.depthAlpha;
          const fromDepth = (typeof td._moveFromDepth === 'number') ? td._moveFromDepth : livePos.depthN;
          ...
          const mt2 = (window as any)._mouseTilt || { mx: window.innerWidth / 2, my: window.innerHeight / 2 };
          const tcx = x + td.TW / 2;
          const tcy = y + td.TH / 2;
          ...
          if (td._lastA !== aV) { td.el.style.opacity = String(aV); td._lastA = aV; }
          const zV = Math.round(fromDepth * 20 + 1);
          if (td._lastZ !== zV) { td.el.style.zIndex = String(zV); td._lastZ = zV; }
          return;
        }
        td.curX = livePos.x; td.curY = livePos.y;
        td._convInit = false;
        const intro = getIntroState(td, idx, ts);
        const introPos = intro.y ? Object.assign({}, livePos, { y: livePos.y + intro.y }) : livePos;
    ...
    function _initPageLinesOnce() {
      ...
      if ((window as any)._initPageLines) { _pageLinesInited = true; (window as any)._initPageLines(); }
    }
    function setProgress(p: number) {
      ...
      if (!(window as any)._thumbAnimDone && wasDone) {
        ...
        if (lcirc) { lcirc.classList.remove('lines-started'); lcirc.style.top = ''; }
        if ((window as any).PageLines && typeof (window as any).PageLines.resetProg === 'function') (window as any).PageLines.resetProg();
      }
    }
    ...
    const BOLT_COUNT = 6, BOLT_SEGS = 9, STEPS = 200;
    ...
    let bolts: any[] = [];
    let _wc = 0, _sc = 0;
    let lineA: any[] = [], lineB: any[] = [], lineC: any[] = [];
    let cardData: any[] = [];
    let _originY = 0, _lastDotY = 0, _btnY = 0;
    let _wanderStartY = 0, _wanderEndY = 0;
    let _pinchTs: number[] = [];
    let drawProg = 0, smoothProg = 0;
    let weldParticles: any[] = [];
    let wA: any[] = [], wB: any[] = [], wC: any[] = [];
    ...
    function initAudio() {
      ...
      try {
        ...
        fetch(SPARK_AUDIO_SRC)
          ...
          .catch(err => console.warn('spark audio buffer load failed, using HTMLAudio fallback:', err));
      } catch (e) { console.warn('AudioContext failed, using HTMLAudio fallback:', e); }
    }
    ...
    function spark() {
      ...
      if (actx && actx.state === 'suspended') actx.resume().then(play).catch(playHtmlSpark);
      else play();
    }
    const onFirstInteraction = () => {
      initAudio();
      if (actx && actx.state === 'suspended') actx.resume().catch(() => {});
      ['pointerdown', 'mousedown', 'click', 'wheel', 'keydown', 'touchstart'].forEach(ev =>
    ...
    function _lerp(ax: number, ay: number, bx: number, by: number, t0: number, t1: number, tc: number) {
      if (Math.abs(t1 - t0) < 1e-10) return [ax, ay];
      const f = (tc - t0) / (t1 - t0);
      return [ax + f * (bx - ax), ay + f * (by - ay)];
    }
    function cr(anch: any[]) {
      const pts: any[] = [], N = anch.length - 1;
      const kn = [0];
      for (let i = 0; i < N; i++) {
        const dx = anch[i + 1].x - anch[i].x, dy = anch[i + 1].y - anch[i].y;
        kn.push(kn[i] + Math.sqrt(dx * dx + dy * dy));
      }
      for (let i = 0; i < N; i++) {
        const p0 = anch[Math.max(0, i - 1)], p1 = anch[i], p2 = anch[i + 1], p3 = anch[Math.min(N, i + 2)];
        const t0 = kn[Math.max(0, i - 1)], t1 = kn[i], t2 = kn[i + 1], t3 = kn[Math.min(N, i + 2)];
      ...
      for (let i = 0; i < pts.length; i++) {
        ...
        const tx = b.x - a.x, ty = b.y - a.y, tl = Math.sqrt(tx * tx + ty * ty) || 1;
    ...
    function build() {
      ...
      const sy = window.scrollY, cx = VW / 2;
      const startY = getStableLineStartY();
      _originY = startY + VH * 0.50;
      cardData = cards.map(c => {
        ...
        return { el: c, dotX, dotY, lineT: 0, opacity: 0, phase: Math.random() * Math.PI * 2 };
      });
      _lastDotY = cardData[cardData.length - 1].dotY;
      cardData.forEach(cd => { cd.lineT = 0; cd._touched = false; });
      ...
      const swings = Array.from({ length: cardData.length }, () => {
        const base = rnd(0.25, 0.70);
        let a = Math.max(0.05, Math.min(0.90, base + rnd(-spread, spread)));
        let b = Math.max(0.05, Math.min(0.90, base + rnd(-spread, spread)));
        let c = Math.max(0.05, Math.min(0.90, base + rnd(-spread, spread)));
        [a, b, c] = [a, b, c].sort((x, y) => x - y);
        if (b - a < MIN) b = a + MIN;
        if (c - b < MIN) c = b + MIN;
        if (c > 0.92) { c = 0.92; b = Math.min(b, c - MIN); a = Math.min(a, b - MIN); }
        return [a, b, c];
      });
      const D = 20;
      const PULL = 0.30;
      ...
      cardData.forEach((c, idx) => {
        const dX = c.dotX, dY = c.dotY;
        const sw = swings[idx % swings.length];
        const mY = prevY + (dY - prevY) * 0.5;
        const rawA = VW * sw[0], rawB = VW * sw[1], rawC = VW * sw[2];
      ...
      lineA = cr(aA); lineB = cr(aB); lineC = cr(aC);
    ...
    function drawLine(pts: any[], prog: number, sx: number) {
      ...
      lctx.save();
      lctx.strokeStyle = 'rgba(48,54,64,1)'; lctx.lineWidth = 1;
      lctx.lineCap = 'round'; lctx.lineJoin = 'round'; lctx.globalAlpha = 1;
      lctx.beginPath();
      let started = false;
      for (let i = 0; i < pts.length; i++) {
        if (pts[i].t > prog) break;
        const px = pts[i].x, sy2 = pts[i].y - sx;
        if (px < -10 || px > VW + 10) { started = false; continue; }
        if (!started) { lctx.moveTo(px, sy2); started = true; }
    ...
    function getTipY() {
      ...
      [wA, wB, wC].forEach(pts => {
        for (let i = pts.length - 1; i >= 0; i--) {
          if (pts[i].t <= drawProg) { if (pts[i].y > tip) tip = pts[i].y; break; }
        }
      });
      return tip;
    }
    function drawCardDots(sx: number) {
      ...
      cardData.forEach(cd => {
        ...
        lctx!.save();
        lctx!.fillStyle = 'rgba(215,215,205,1)'; lctx!.globalAlpha = 0.55;
        lctx!.beginPath(); lctx!.arc(cd.dotX, sy2, 1.5, 0, Math.PI * 2); lctx!.fill();
        lctx!.restore();
      });
    }
    function drawFrontDots(sx: number) {
      ...
      [wA, wB, wC].forEach(pts => {
        ...
        let fp = pts[0];
        for (let i = pts.length - 1; i >= 0; i--) { if (pts[i].t <= drawProg) { fp = pts[i]; break; } }
        const sy2 = fp.y - sx;
        if (sy2 < -10 || sy2 > VH + 10) return;
        if (fp.x < 0 || fp.x > VW) return;
        lctx!.save();
        lctx!.fillStyle = 'rgba(200,200,200,1)'; lctx!.globalAlpha = 0.85;
        lctx!.beginPath(); lctx!.arc(fp.x, sy2, 1.5, 0, Math.PI * 2); lctx!.fill();
        lctx!.restore();
      });
    }
    const DOT_BOLT_COUNT = 8;
    const dotBolts: any[] = [];
    function mkBolt() { return { pts: [], life: 0, max: 0, on: false, col: 0x88ccff } as any; }
    ...
    function spawnWeldSparks(ox: number, oy: number) {
      ...
      for (let i = 0; i < COUNT; i++) {
        ...
        const slot = dotBolts.find(b => !b.on) || dotBolts[i % DOT_BOLT_COUNT];
        spawnBoltOn(slot, ox, oy, tx, ty);
        slot.life += Math.random() * 0.03;
        slot.max = slot.life;
      }
      weldParticles.push({ flash: true, x: ox, y: oy, life: 0.10, maxLife: 0.10 });
    }
    function drawWeldSparks(dt: number, sx: number) {
      ...
      weldParticles = weldParticles.filter(p => {
        ...
        const a = fade * 0.75;
        lctx!.save();
        const g = lctx!.createRadialGradient(p.x, p.y - sx, 0, p.x, p.y - sx, Math.max(0.1, r));
        g.addColorStop(0, `rgba(255,255,255,${a})`);
        g.addColorStop(0.5, `rgba(53,135,242,${a * 0.5})`);
        g.addColorStop(1, 'rgba(0,0,0,0)');
        lctx!.fillStyle = g;
        lctx!.beginPath(); lctx!.arc(p.x, p.y - sx, Math.max(0.1, r), 0, Math.PI * 2); lctx!.fill();
        lctx!.restore();
        return true;
      });
      dotBolts.forEach(b => {
        if (!b.on) return; b.life -= dt; if (b.life <= 0) { b.on = false; return; }
        const sp = b.pts.map((p: any) => ({ x: p.x, y: p.y - sx }));
        const fade = b.life / b.max;
        const dx0 = sp[sp.length - 1].x - sp[0].x, dy0 = sp[sp.length - 1].y - sp[0].y;
        const blen = Math.sqrt(dx0 * dx0 + dy0 * dy0) || 1;
        const s = Math.min(1, blen / 80);
        lctx!.save(); lctx!.filter = `blur(${(2 + s * 2).toFixed(1)}px)`;
        lctx!.strokeStyle = `rgba(80,125,181,${fade})`; lctx!.lineWidth = 1.5 + s * 1.5;
        lctx!.lineCap = 'round'; lctx!.lineJoin = 'round'; lctx!.beginPath();
        sp.forEach((p: any, i: number) => i === 0 ? lctx!.moveTo(p.x, p.y) : lctx!.lineTo(p.x, p.y));
        lctx!.stroke(); lctx!.restore();
        lctx!.save(); lctx!.filter = `blur(${(0.8 + s * 0.5).toFixed(1)}px)`;
        lctx!.strokeStyle = `rgba(53,135,242,${0.75 * fade})`; lctx!.lineWidth = 0.8 + s * 0.8;
        lctx!.lineCap = 'round'; lctx!.lineJoin = 'round'; lctx!.beginPath();
        sp.forEach((p: any, i: number) => i === 0 ? lctx!.moveTo(p.x, p.y) : lctx!.lineTo(p.x, p.y));
        lctx!.stroke(); lctx!.restore();
        lctx!.save(); lctx!.filter = 'blur(0px)';
        lctx!.strokeStyle = `rgba(255,255,255,${fade})`; lctx!.lineWidth = 0.7;
        lctx!.lineCap = 'round'; lctx!.lineJoin = 'round'; lctx!.beginPath();
        sp.forEach((p: any, i: number) => i === 0 ? lctx!.moveTo(p.x, p.y) : lctx!.lineTo(p.x, p.y));
        lctx!.stroke(); lctx!.restore();
      });
    ...
    const BCOLS = [0xffffff, 0x88ddff, 0x44aaff, 0x0066ff, 0x00ccff, 0xaaddff, 0x0044cc];
    function spawnBoltOn(b: any, ox: number, oy: number, tx: number, ty: number) {
      const dx = tx - ox, dy = ty - oy, dist = Math.sqrt(dx * dx + dy * dy) || 0.001;
      const px = -dy / dist, py = dx / dist, od = Math.random() > 0.5 ? 1 : -1;
      const pts: any[] = [{ x: ox, y: oy }];
      let cx = ox, cy = oy;
      for (let i = 1; i <= BOLT_SEGS; i++) {
        const t = i / BOLT_SEGS;
        const jMag = Math.min(dist * 0.30, 13) * (0.5 + Math.random() * 0.8) * Math.sin(t * Math.PI);
        const oMag = Math.min(dist * 0.12, 6) * Math.sin(t * Math.PI) * od;
        const nx = ox + dx * t + (Math.random() - 0.5) * jMag * 1.4 + px * oMag;
        const ny = oy + dy * t + (Math.random() - 0.5) * jMag + py * oMag * (0.6 + Math.random() * 0.8);
        pts.push({ x: nx, y: ny });
        if (i === Math.floor(BOLT_SEGS * 0.45)) { cx = nx; cy = ny; }
      }
      pts.push({ x: tx, y: ty });
      b.pts = pts;
      if (Math.random() < 0.55) {
        ...
        const fx = cx + Math.cos(fAngle) * fLen, fy = cy + Math.sin(fAngle) * fLen;
        const bPts: any[] = [{ x: cx, y: cy }];
        for (let i = 1; i <= 4; i++) {
          const t = i / 4;
          bPts.push({ x: cx + (fx - cx) * t + (Math.random() - 0.5) * 4, y: cy + (fy - cy) * t + (Math.random() - 0.5) * 4 });
        }
        bPts.push({ x: fx, y: fy });
        b.branch = bPts;
      } else { b.branch = null; }
      b.max = 0.04 + Math.random() * 0.06; b.life = b.max; b.on = true;
      b.col = BCOLS[Math.floor(Math.random() * BCOLS.length)];
    }
    function drawBolts(dt: number, sx: number) {
      if (!lctx) return;
      _sc = Math.max(0, _sc - dt); _wc = Math.max(0, _wc - dt);
      bolts.forEach(b => {
        if (!b.on) return; b.life -= dt; if (b.life <= 0) { b.on = false; return; }
        const sp = b.pts.map((p: any) => ({ x: p.x, y: p.y - sx }));
        const minY = Math.min(...sp.map((p: any) => p.y)), maxY = Math.max(...sp.map((p: any) => p.y));
        ...
        lctx!.lineWidth = 2 + s * 2; lctx!.lineCap = 'round'; lctx!.lineJoin = 'round';
        lctx!.beginPath();
        sp.forEach((p: any, i: number) => i === 0 ? lctx!.moveTo(p.x, p.y) : lctx!.lineTo(p.x, p.y));
        lctx!.stroke(); lctx!.restore();
        lctx!.save();
        lctx!.filter = `blur(${(1 + s * 1).toFixed(1)}px)`;
        lctx!.strokeStyle = `rgba(53,135,242,${0.70 * fade})`;
        lctx!.lineWidth = 1 + s * 1.2; lctx!.lineCap = 'round'; lctx!.lineJoin = 'round';
        lctx!.beginPath();
        sp.forEach((p: any, i: number) => i === 0 ? lctx!.moveTo(p.x, p.y) : lctx!.lineTo(p.x, p.y));
        lctx!.stroke(); lctx!.restore();
        lctx!.save();
        ...
        lctx!.lineWidth = 0.9; lctx!.lineCap = 'round'; lctx!.lineJoin = 'round';
        lctx!.beginPath();
        sp.forEach((p: any, i: number) => i === 0 ? lctx!.moveTo(p.x, p.y) : lctx!.lineTo(p.x, p.y));
        lctx!.stroke(); lctx!.restore();
        if (b.branch) {
          const bp = b.branch.map((p: any) => ({ x: p.x, y: p.y - sx }));
          lctx!.save(); lctx!.filter = `blur(${(1.5 + s).toFixed(1)}px)`;
          lctx!.strokeStyle = `rgba(80,125,181,${0.7 * fade})`; lctx!.lineWidth = 1 + s * 0.8;
          lctx!.lineCap = 'round'; lctx!.lineJoin = 'round'; lctx!.beginPath();
          bp.forEach((p: any, i: number) => i === 0 ? lctx!.moveTo(p.x, p.y) : lctx!.lineTo(p.x, p.y));
          lctx!.stroke(); lctx!.restore();
          lctx!.save(); lctx!.filter = 'blur(0px)';
          lctx!.strokeStyle = `rgba(255,255,255,${0.7 * fade})`; lctx!.lineWidth = 0.5;
          lctx!.lineCap = 'round'; lctx!.lineJoin = 'round'; lctx!.beginPath();
          bp.forEach((p: any, i: number) => i === 0 ? lctx!.moveTo(p.x, p.y) : lctx!.lineTo(p.x, p.y));
          lctx!.stroke(); lctx!.restore();
        }
    ...
    function nearLine(pts: any[], mx: number, my: number, sx: number, th: number) {
      let best: any = null, bd = th * th;
      for (let i = 0; i < pts.length; i++) {
        if (pts[i].t > drawProg) break;
        const dy2 = pts[i].y - sx - my, dx2 = pts[i].x - mx, d = dx2 * dx2 + dy2 * dy2;
    ...
    function checkWeld(sx: number) {
      ...
      const all = [wA, wB, wC]; let hitL = -1, hitP: any = null;
      for (let li = 0; li < 3; li++) { const p = nearLine(all[li], _mx, _my, sx, 18); if (p) { hitL = li; hitP = p; break; } }
      if (!hitP) return;
      ...
      bolts.forEach(b => {
        ...
        for (let i = 0; i < tgtLine.length; i++) {
          ...
          const dx2 = tgtLine[i].x - hitP.x, dy2 = tgtLine[i].y - hitP.y, d = dx2 * dx2 + dy2 * dy2;
          if (d < bd) { bd = d; best = tgtLine[i]; }
        }
        if (best) spawnBoltOn(b, hitP.x, hitP.y, best.x, best.y);
      });
      spark(); _wc = 0.1 + Math.random() * 0.2;
    }
    const onMouseMove = (e: MouseEvent) => { _mx = e.clientX; _my = e.clientY; };
    document.addEventListener('mousemove', onMouseMove);

    function updateProg() {
      const sy = window.scrollY;
      if (!lineA.length) { drawProg = 0; smoothProg = 0; return; }
      const startY = Math.max(0, _originY - VH * 0.50);
      const endY   = Math.max(startY + 1, _btnY - VH * 0.50);
    ...
    function plLoop(ts: number) {
      ...
      const dt = Math.min((ts - plLast) / 1000, 0.05); plLast = ts; _t += dt;
      updateProg();
      const sx = window.scrollY;
      wA = wave(lineA, 1.0); wB = wave(lineB, 2.3); wC = wave(lineC, 3.7);
      if (!lctx) return;
      lctx.clearRect(0, 0, VW, VH);
      lctx.save();
      lctx.beginPath(); lctx.rect(0, 0, VW, VH); lctx.clip();
      drawLine(wA, drawProg, sx); drawLine(wB, drawProg, sx); drawLine(wC, drawProg, sx);
      drawCardDots(sx);
      drawFrontDots(sx);
      lctx.restore();
      updateCards();
    ...
    function plInit() {
      ...
      if (plRafId) { cancelAnimationFrame(plRafId); plRafId = null; }
      if (plResizeHandler) window.removeEventListener('resize', plResizeHandler);
      bolts = [];
      for (let i = 0; i < BOLT_COUNT; i++) bolts.push(mkBolt());
      document.querySelectorAll<HTMLElement>('.project-card').forEach(c => { c.style.opacity = '0'; c.style.transition = 'none'; });
      plResize();
      plResizeHandler = debounce(plResize, 200);
      window.addEventListener('resize', plResizeHandler);
      plRafId = requestAnimationFrame(ts => { plLast = ts; plRafId = requestAnimationFrame(plLoop); });
    }
    function resetProg() { drawProg = 0; smoothProg = 0; }
    (window as any).PageLines = { init: plInit, rebuild: build, resetProg };
    (window as any)._initPageLines = plInit;

    // ===== boot intro on load =====
    (window as any)._mouseTilt = { mx: window.innerWidth / 2, my: window.innerHeight / 2 };
    ...
    function startIntro() {
      ...
      if (lc0) { lc0.style.opacity = '1'; lc0.style.visibility = 'visible'; lc0.style.transform = 'perspective(800px)'; }
      (window as any)._thumbAnimReady = true;
      (window as any)._thumbAnimDone = false;
      updateDotPos();
      placeThumbs();
      requestAnimationFrame(ts => { lastTs = ts; introRafId = requestAnimationFrame(loop); });
      bootScrollTrigger();
    }
  ...
  return (
    <div className="bg-[#0A0A0A] text-white relative overflow-x-hidden font-sans">
      <style>{styles}</style>

      <div id="header-dot" />

      <div id="page-header">
        <div id="page-header-pin">
          <div id="floating-thumbs">
            {FT_THUMBS.map((src, i) => (
              <div key={i} className="ft"><img src={src} alt="" /></div>
            ))}
          </div>
          <div id="logo-circle">
            <div id="logo-svg-wrap">
              <svg className="logo-t-path" id="t1" viewBox="0 0 578 586" fill="none" xmlns="http://www.w3.org/2000/svg">
                <defs>
                  <clipPath id="clip1">
                ...
                <path fillRule="evenodd" clipRule="evenodd" d="M269.431 257.019C268.708 258.271 268.717 259.816 269.455 261.06L286.98 290.611C288.54 293.241 292.355 293.219 293.885 290.57L327.284 232.72C328.825 230.052 332.677 230.054 334.215 232.724L425.279 390.875C425.982 392.095 427.275 392.856 428.682 392.878L463.65 393.434C466.755 393.483 468.73 390.129 467.18 387.438L353.861 190.681C353.148 189.444 353.149 187.921 353.863 186.685L388.264 127.101C388.965 125.885 388.979 124.391 388.299 123.163L371.368 92.5833C369.861 89.8615 365.96 89.8265 364.404 92.5208L269.431 257.019Z" fill="none" stroke="#303640" strokeWidth="1.5" strokeOpacity="0.7" />
                <g clipPath="url(#clip1)" id="particles1" />
                <g clipPath="url(#clip1)" id="lightning1" />
              </svg>
              <svg className="logo-t-path" id="t2" viewBox="0 0 578 586" fill="none" xmlns="http://www.w3.org/2000/svg">
                <defs>
                  <clipPath id="clip2">
                ...
                <path fillRule="evenodd" clipRule="evenodd" d="M316.66 425.547C318.087 425.545 319.406 426.304 320.123 427.537L356.868 490.777C357.572 491.989 358.86 492.744 360.263 492.767L395.242 493.322C398.347 493.372 400.322 490.018 398.773 487.327L304.909 324.268C304.196 323.03 302.878 322.266 301.449 322.263L265.933 322.201C262.846 322.196 260.916 325.543 262.468 328.212L293.454 381.508C295.005 384.176 293.079 387.521 289.993 387.519L107.1 387.384C105.67 387.383 104.348 388.146 103.633 389.384L86.0729 419.792C84.532 422.461 86.4598 425.796 89.5411 425.793L316.66 425.547Z" fill="none" stroke="#303640" strokeWidth="1.5" strokeOpacity="0.7" />
                <g clipPath="url(#clip2)" id="particles2" />
                <g clipPath="url(#clip2)" id="lightning2" />
              </svg>
              <svg className="logo-t-path" id="t3" viewBox="0 0 578 586" fill="none" xmlns="http://www.w3.org/2000/svg">
                <defs>
                  <clipPath id="clip3">
                ...
                <path fillRule="evenodd" clipRule="evenodd" d="M182.809 283.818C179.736 283.807 177.822 280.479 179.359 277.818L271.364 118.461C272.066 117.245 272.079 115.751 271.399 114.523L254.468 83.9432C252.961 81.2213 249.06 81.1863 247.504 83.8807L133.245 281.783C132.528 283.026 131.2 283.789 129.765 283.783L56.9478 283.499C55.5132 283.494 54.1855 284.257 53.4681 285.499L35.9048 315.92C34.3665 318.584 36.2864 321.915 39.363 321.92L229.336 322.201C230.767 322.203 232.091 321.441 232.806 320.201L250.222 290.036C251.759 287.375 249.845 284.047 246.771 284.036L182.809 283.818Z" fill="none" stroke="#303640" strokeWidth="1.5" strokeOpacity="0.7" />
                <g clipPath="url(#clip3)" id="particles3" />
                <g clipPath="url(#clip3)" id="lightning3" />
              </svg>
            </div>
          </div>
          <h1 id="page-h1">Our work</h1>
          <div id="line-origin" />
          <p id="page-sub">A curated collection of branding,<br />web, and digital system work.</p>
        </div>
      </div>
      ...
      <main id="projects-grid">
        {PROJECTS.map((p, i) => (
          <article key={i} className="project-card" data-pos={p.pos} data-size={p.size}>
            <div className="card-thumb">
              {p.link ? (
                <a href="#" className="thumb-link" aria-label={`Open ${p.title}`}>
            ...
            <div className="card-info">
              ...
              <div className="card-right-meta">
                ...
                <a href="#" className="cta">EXPLORE PROJECT <span>→</span></a>
              </div>
            </div>
          </article>
        ))}
      ...
      <div id="contact-section">
        ...
        <div id="contact-subtext">
          <p>Every project begins with a conversation — let&apos;s start yours.</p>
          <p>We work with studios, founders, and brands who care about craft.</p>
...
.ft { position: absolute; width: 170px; border-radius: 4px; overflow: hidden; opacity: 0; will-change: transform, opacity, left, top; }
